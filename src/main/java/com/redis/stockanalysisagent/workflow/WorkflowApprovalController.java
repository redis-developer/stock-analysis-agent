...
public class WorkflowApprovalController {
    ...
    private ChatResponse decide(String workflowId, String approvalId, boolean approved, HttpServletRequest httpRequest) {
        ...
        String executionLockKey = "approval:" + approval.approvalId() + ":" + approval.status();
        Optional<WorkflowService.ExecutionLock> lock = workflowService.tryAcquireExecutionLock(executionLockKey);
        if (lock.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Approval resume is already running.");
        }
